...
from utils.gdla_preprocess import get_lonlat
    ...
    if polygon:
        ...
        for path, rgb_band, div_img_list, div_coord, shapes in dataset:  
                    ...
                    if len(det):
                        ...
                        for *xyxyxyxy, conf, cls in reversed(det):
                            if save_txt:  # Write to file
                                ...
                                xyxyxyxy = [el*div_coord[d_id][0]  if i%2==0 else el*div_coord[d_id][1]  for i, el in enumerate(xyxyxyxyn)]
                                xyxyxyxy = [el+div_coord[d_id][2]  if i%2==0 else el+div_coord[d_id][3]  for i, el in enumerate(xyxyxyxy)]
                                
                                xyxyxyxy = [int(el) for el in xyxyxyxy]
                                line = (cls, *xyxyxyxy, conf) if save_conf else (cls, *xyxyxyxy)  # label format
                                lon, lat = get_lonlat(xyxyxyxy)
                                

                                with open(save_dir / (p.stem + '_BRIDG_RGB000102.txt'), 'a') as f:
                                    f.write(('%g ' * len(line)).rstrip() % line + '\n')
                                with open(save_dir / (p.stem + '_BRIDG_RGB000102_poly.txt'), 'a') as f:
                                    f.write(('%g ' * len(line)).rstrip() % line + '\n')
            ...
            if save_img or view_img:
                 save_path = str(save_dir / Path(path).name.replace('.tif', '_BRIDG_RGB000102.tif'))
            
                 print(f" The image with the result is saved in: {save_path}")
                 rgb_band = np.array(rgb_band, np.uint8)
                 cv2.imwrite(save_path, rgb_band)   # find what to plot             
