...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 18 17:18:19 2023
...
from utils.datasets_solo import create_dataloader
from utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    xyxy2xywh, xywh2xyxy, set_logging, increment_path, \
    colorstr, polygon_box_iou, polygon_non_max_suppression, polygon_scale_coords 
from utils.metrics import ap_per_class, Polygon_ConfusionMatrix
from utils.plots import plot_study_txt, polygon_plot_images, polygon_output_to_target, set_colors
...
def process_batch(detections, labels, iouv):
    ...
    iou = polygon_box_iou(labels[:, 1:], detections[:, :4])
...
def test(data,
    ...
    if training:  # called by train.py
        ...
        save_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok)+\
                        f'_confth{opt.conf_thres}_iouth{opt.iou_thres}')  # increment run
    ...
    if not training:
        ...
        task = opt.task if opt.task in ('train', 'valid', 'test') else 'valid'  # path to train/val/test images
        dataloader = create_dataloader(data[task], imgsz, batch_size, gs, opt, pad=0.5, rect=True,
                                       prefix=colorstr(f'{task}: '), polygon=polygon)[0]
    seen = 0
        ...
        # Statistics per image
            ...
            for si, pred in enumerate(out):
                ...
                if save_json:
                    ...
                    x = box[:, 0::2]
                    y = box[:, 1::2]
                    xmin = x.min(dim=1)[0]; xmax = x.max(dim=1)[0]
                    ymin = y.min(dim=1)[0]; ymax = y.max(dim=1)[0]
                    box = torch.cat((xmin.unsqueeze(0).T, ymin.unsqueeze(0).T,xmax.unsqueeze(0).T, ymax.unsqueeze(0).T), axis=1)
    ...
    if save_json and len(jdict):
        ...
        anno_json = './data/sentinel_valid_annotations.json'  # annotations json
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            eval.params.iouThrs = iouv.detach().cpu()#np.linspace(.3, 0.95, int(np.round((0.95 - .3) / .05)) + 1, endpoint=True)
            eval.params.areaRng = [[0 ** 3, 1e5 ** 2], [0 ** 3, 10 ** 2], [10 ** 2, 100 ** 2], [100 ** 2, 1e5 ** 2]]
            if is_coco:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/sentinel.yaml', help='*.data path')
    parser.add_argument('--batch-size', type=int, default=16, help='size of each image batch')
    parser.add_argument('--img-size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.001, help='object confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.3, help='IOU threshold for NMS')
    parser.add_argument('--task', default='valid', help='train, valid, test, speed or study')
    ...
    parser.add_argument('--polygon', action='store_true', help='enable polygon anchor boxes') # CHange~~
    ...
    if opt.task in ('train', 'valid', 'test'):  # run normally
        test(opt.data,
             opt.weights,
             opt.batch_size,
             opt.img_size,
        ...
        for w in opt.weights:
            ...
            for i in x:  # img-size
                print(f'\nRunning {f} point {i}...')
                r, _, t = test(opt.data, w, opt.batch_size, i, opt.conf_thres, opt.iou_thres, 
                               opt.save_json, plots=False, polygon=opt.polygon)
