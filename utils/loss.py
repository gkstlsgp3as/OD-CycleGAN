...
from utils.general import bbox_iou, xywh2xyxy, polygon_bbox_iou, order_corners, wh_iou, box_iou
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        #for rotated: BCEtheta = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['theta_pw']], device=device))
        ...
        if g > 0:
            ...
            #for rotated: BCEtheta = FocalLoss(BCEtheta, g)
        ...
        #for rotated: self.BCEtheta = BCEtheta
    ...
    # original
    def build_targets(self, p, targets): # p: predictions 
        
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        tcls, tbox, indices, anch = [], [], [], []
        gain = torch.ones(7, device=targets.device).long()  # normalized to gridspace gain => img_id, cls_id, xywh, anc_id 
...
class ComputeLossBinOTA:
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            pair_wise_iou = bbox_iou(txyxy, pxyxys)
...
if __name__ == '__main__':
    ...
    arr1 = np.array([0.00000, 0.00000, 0.31390, 0.00510, 0.48840, -0.00885, 0.49189, 0.04000, 0.31739, 0.05395, 0.00000])
    arr2 = np.array([0.00000, 0.00000, 0.57695, 0.10829, 0.60567, 0.10255, 0.62674, 0.20966, 0.59993, 0.21540, 0.00000])
    labels = torch.stack([torch.from_numpy(arr1), torch.from_numpy(arr2)], dim=0) #(2, 11)
    
    pred = torch.rand((1, 3, 64, 64, 10))
