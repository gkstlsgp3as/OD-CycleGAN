...
class LoadSAR:
    ...
    def __next__(self):
        ...
        from utils.gdal_preprocess import band_to_rgb, line_detection, division_testset
        ...
        # RGB로 변환
        if Cfg.NewTest == 0:
            rgb_band = band_to_rgb(path, Cfg.Satelliteband)
        else:
            rgb_band = band_to_rgb(path, Cfg.Satelliteband, True)
        ...
        return path, rgb_band, div_img_list, div_coord, rgb_band.shape
                    ...
                    if nl:
                        ...
                        if (l < 0).any(): # unnormalized labels
                            ...
                            if l.shape[0]==0:
                                l = np.zeros((0,9), dtype=np.float32)
                        # shhan add
                        if (l[:, 1:] > 1).any(): # unnormalized labels
