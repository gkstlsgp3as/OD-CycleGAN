...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    wh = torch.tensor(np.concatenate([l[:, 3:5] * s for s, l in zip(shapes * scale, dataset.labels) if len(l)>0])).float()  # wh
...
def polygon_check_anchors(dataset, model, thr=4.0, imgsz=640):
    # Check anchor fit to data, recompute if necessary
    prefix = colorstr('autoanchor: ')
    print(f'\n{prefix}Analyzing anchors for polygon... ', end='')
    m = model.module.model[-1] if hasattr(model, 'module') else model.model[-1]  # Polygon_Detect()
    shapes = imgsz * dataset.shapes / dataset.shapes.max(1, keepdims=True)
    scale = np.random.uniform(0.9, 1.1, size=(shapes.shape[0], 1))  # augment scale
    ...
    if bpr < 0.99:  # threshold to recompute
        ...
        na = m.anchor_grid.numel() // 2  # number of anchors
        try:
            anchors = polygon_kmean_anchors(dataset, n=na, img_size=imgsz, thr=thr, gen=1000, verbose=False)
        except Exception as e:
            print(f'{prefix}ERROR: {e}')
        new_bpr = metric(anchors)[0]
        if new_bpr > bpr:  # replace anchors
...
def polygon_kmean_anchors(path='./data/ssdd.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    if isinstance(path, str):  # *.yaml file
        with open(path) as f:
            data_dict = yaml.safe_load(f)  # model dict
        from utils.datasets import Polygon_LoadImagesAndLabels
        dataset = Polygon_LoadImagesAndLabels(data_dict['train'], augment=True, rect=True, yaml_file=path.split('/')[-1])
    else:
        dataset = path  # dataset
