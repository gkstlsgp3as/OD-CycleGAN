...
def set_colors(names, target):
    ...
    if target == 'SHIPD':
        colors[names.index('ship')] = Cfg.color[0] # red
    elif target == "BRIDG":
        for tg in names:
            colors[names.index(tg)] = Cfg.color[names.index(tg)+1] # 'veh', 'trn', 'prd' => green, blue, purple
...
def plot_images(images, targets, colors, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=img.dtype)  # init
...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    for kid in range(num_kpts):
        ...
        if not (x_coord % img_sz == 0 or y_coord % img_sz == 0):
            if steps == 3:
    ...
    for sk_id, sk in enumerate(skeleton):
        ...
        if steps == 3:
            ...
            if conf1<0.5 or conf2<0.5:
                continue
        if pos1[0]%img_sz == 0 or pos1[1]%img_sz==0 or pos1[0]<0 or pos1[1]<0:
            continue
        if pos2[0] % img_sz == 0 or pos2[1] % img_sz == 0 or pos2[0]<0 or pos2[1]<0:
            continue
...
def polygon_plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    if label:
        ...
        cv2.putText(im, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA) #Change
...
def polygon_plot_one_box_PIL(x, im, color=(128, 128, 128), label=None, line_thickness=None):
    ...
    if label:
        ...
        txt_width, txt_height = font.getsize(label)
        c = (box[0::2].mean(), box.ravel()[1::2].mean())
        draw.rectangle([c[0], c[1] - txt_height + 4, c[0] + txt_width, c[1]], fill=color)
        draw.text((c[0], c[1] - txt_height + 1), label, fill=(255, 255, 255), font=font)
...
def polygon_output_to_target(output):
    ...
    for i, o in enumerate(output):
        for *box, conf, cls in o.cpu().numpy():
            targets.append([i, cls, *list(box), conf])
    return np.array(targets)


def polygon_plot_images(images, targets, colors, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    # Plot image grid with labels
    if isinstance(images, torch.Tensor):
        images = images.cpu().float().numpy()
    if isinstance(targets, torch.Tensor):
    ...
    if fname:
        ra = min(1280. / max(h, w) / ns, 1.0)  # ratio to limit image size
        mosaic = cv2.resize(mosaic, (int(ns * w * ra), int(ns * h * ra)), interpolation=cv2.INTER_AREA)
        # cv2.imwrite(fname, cv2.cvtColor(mosaic, cv2.COLOR_BGR2RGB))  # cv2 save
...
def polygon_plot_targets_txt():  # from utils.plots import *; polygon_plot_targets_txt()
    # Plot targets.txt histograms
    x = np.loadtxt('targets.txt', dtype=np.float32).T
...
def polygon_plot_labels(labels, colors, names=(), save_dir=Path(''), loggers=None):
    # plot dataset labels
    print('Plotting labels... ')
    c, b = labels[:, 0], labels[:, 1:].transpose()  # classes, boxes
    nc = int(c.max() + 1)  # number of classes
    ...
    for cls, *box in labels[:1000]:
        hexcolor = '#%02x%02x%02x' % tuple(colors[int(cls)])
        ImageDraw.Draw(img).polygon(box, outline=hexcolor)  # plot
    ax[1].imshow(img)
    ax[1].axis('off')
