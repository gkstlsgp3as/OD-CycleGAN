...
from utils.general import non_max_suppression, polygon_scale_coords, make_divisible, \
	scale_coords, increment_path, xyxy2xywh, polygon_non_max_suppression
...
class Conv2(nn.Module):
	# Standard convolution
	def __init__(self, c1, c2, k=1, s=1, index=0, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
	...
	def forward(self, x):
		return self.act(self.bn(self.conv(x[self.index])))

	def fuseforward(self, x):
		return self.act(self.conv(x[self.index]))
...
class Mult2(nn.Module):
	...
	def forward(self, x):
		if self.index == 0:
			return torch.matmul(x[0], x[1][0])
		elif self.index == 1:
			return torch.matmul(x[0], x[1][1])
		# return torch.add(x[0], x[1])
...
class Fusion_TransformerBlock(nn.Module):
	""" Transformer block """
...
class ChannelGate(nn.Module):
	def __init__(self, c1, reduction_ratio=16):
		...
		self.mlp = nn.Sequential(
			...
			nn.ReLU(),
			nn.Linear(c1 // reduction_ratio, c1),
			nn.Sigmoid()
			)
	...
	def forward(self, x):
		...
		attention = x_avg_pool#torch.where(x_avg_pool > x_avg_pool.mean(), x_avg_pool, 0.0)
...
class SpatialGate(nn.Module):
	def __init__(self, c1):
		...
		self.conv2 = Conv(c1*2, c1, 1, 1)
	def forward(self, x):
		x_avg_pool = torch.mean(x,1).unsqueeze(1) # 1, 1, 128, 128
		x_max_pool = torch.max(x,1)[0].unsqueeze(1)
		attention = torch.cat((x_avg_pool, x_max_pool), dim=1)
		attention = self.conv(attention)
		return self.conv2(x)*attention

class CAM(nn.Module):
	def __init__(self, c1, reduction_ratio):
		super(CAM, self).__init__()
		self.c1 = c1*2
		self.reduction_ratio = reduction_ratio
		self.mid_c = int(self.c1/ float(self.reduction_ratio))

		self.bottleneck = nn.Sequential(
			nn.Linear(self.c1, self.mid_c),
			nn.ReLU(),
			nn.Linear(self.mid_c, self.c1)
		)
	...
	def forward(self, x):
		...
		attention = sig_pool.repeat(1,1,kernel[0], kernel[1])
		x = x*attention
		return x[:,:self.c1//2,:,:], x[:,self.c1//2:,:,:]
...
class CAM2(nn.Module):
	...
	def forward(self, x):
		kernel = (x[0].size()[2], x[0].size()[3])
		avg_pool1 = F.avg_pool2d(x[0], kernel)
		max_pool1 = F.max_pool2d(x[0], kernel)

		avg_pool1 = avg_pool1.view(avg_pool1.size()[0], -1)
		max_pool1 = max_pool1.view(max_pool1.size()[0], -1)
		
		avg_pool_bck1 = self.bottleneck(avg_pool1)
		max_pool_bck1 = self.bottleneck(max_pool1)

		pool_sum1 = avg_pool_bck1 + max_pool_bck1

		sig_pool1 = torch.sigmoid(pool_sum1)
		sig_pool1 = sig_pool1.unsqueeze(2).unsqueeze(3)

		attention1 = sig_pool1.repeat(1,1,kernel[0], kernel[1])
		x[0] = x[0]*attention1

		kernel = (x[1].size()[2], x[1].size()[3])
		avg_pool2 = F.avg_pool2d(x[1], kernel)
		max_pool2 = F.max_pool2d(x[1], kernel)

		avg_pool2 = avg_pool2.view(avg_pool2.size()[0], -1)
		max_pool2 = max_pool2.view(max_pool2.size()[0], -1)
		
		avg_pool_bck2 = self.bottleneck(avg_pool2)
		max_pool_bck2 = self.bottleneck(max_pool2)

		pool_sum2 = avg_pool_bck2 + max_pool_bck2

		sig_pool2 = torch.sigmoid(pool_sum2)
		sig_pool2 = sig_pool2.unsqueeze(2).unsqueeze(3)

		attention2 = sig_pool2.repeat(1,1,kernel[0], kernel[1])
		x[1] = x[1]*attention2
...
class SAM2(nn.Module):
	...
	def forward(self, x):
		...
		x_avg_pool1 = torch.mean(x[0],1).unsqueeze(1) # 1, 1, 128, 128
		x_aux = self.m(x[1])
		x_avg_pool2 = torch.mean(x_aux,1).unsqueeze(1)
		attention = torch.cat((x_avg_pool1, x_avg_pool2), dim=1)
		attention = self.conv(attention)
...
class SAM(nn.Module):
	...
	def forward(self, x):
		x_avg_pool1 = torch.mean(x[0],1).unsqueeze(1) # 1, 1, 128, 128
		x_avg_pool2 = torch.mean(x[1],1).unsqueeze(1)
		attention = torch.cat((x_avg_pool1, x_avg_pool2), dim=1)
		attention = self.conv(attention)
		return x[0]*attention
	
class CBAM_F(nn.Module): #CBAM for multi-modal feature fusion 
	def __init__(self, c1, reduction_ratio=16, channel_attention=True, spatial_attention=True):
		super(CBAM_F, self).__init__()
		self.channel_attention, self.spatial_attention = channel_attention, spatial_attention
		if channel_attention:
			self.ChannelGate = CAM2(c1, reduction_ratio)
		if spatial_attention:
			self.SpatialGate = SAM(c1)

	def forward(self, x):
		if self.channel_attention:
			x = self.ChannelGate(x) # return channel-wise attention
		if self.spatial_attention:
			x = self.SpatialGate(x)
		return x

class CBAM_F2(nn.Module): #CBAM for multi-modal feature fusion 
	def __init__(self, c1, reduction_ratio=16, channel_attention=True, spatial_attention=True):
		super(CBAM_F2, self).__init__()
		self.channel_attention, self.spatial_attention = channel_attention, spatial_attention
		if channel_attention:
			self.ChannelGate = ChannelGate(c1, reduction_ratio)
		if spatial_attention:
			self.SpatialGate = SpatialGate(c1)

	def forward(self, x):
		if self.channel_attention:
			x1 = x[0]  # im1_fea (tensor): dim:(B, C, H, W) ex. 1, 64, 128, 128
			x2 = x[1]
			attn1 = self.ChannelGate(x1) # return channel-wise attention
			attn2 = self.ChannelGate(x2)
			x = torch.cat((x1*attn1, x2*attn2), dim=1) # 1, 128, 128, 128
		if self.spatial_attention:
			x = self.SpatialGate(x)
		return x

class CBAM(nn.Module):
	def __init__(self, c1, c2, reduction_ratio=16, channel_attention=True, spatial_attention=True):
...
class iAFF_F(nn.Module):
	...
	def forward(self, x):
		...
		xlg = xl + xg
		wei = self.sigmoid(xlg)
		xi = x[0] * wei + residual * (1 - wei)

		xl2 = self.local_att2(xi)
		xg2 = self.global_att(xi)
		xlg2 = xl2 + xg2
		wei2 = self.sigmoid(xlg2)
		xo1 = x[0] * wei2 + residual * (1 - wei2)
		...
		xlg = xl + xg
		wei = self.sigmoid(xlg)
		xi = x[1] * wei + residual * (1 - wei)

		xl2 = self.local_att2(xi)
		xg2 = self.global_att(xi)
		xlg2 = xl2 + xg2
		wei2 = self.sigmoid(xlg2)
		xo2 = x[1] * wei2 + residual * (1 - wei2)
		'''
		return wei2, (1-wei2)
...
class GPT(nn.Module):
	...
	def __init__(self, d_model, h=8, block_exp=4,
		...
		# positional embedding parameter (learnable), im1_fea + im2_fea
		self.pos_emb = nn.Parameter(torch.zeros(1, 2 * vert_anchors * horz_anchors, self.n_embd))

		# transformer
		self.trans_blocks = nn.Sequential(*[Fusion_TransformerBlock(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop)
	...
	def forward(self, x):
		...
		"""
		im1_fea = x[0]  # im1_fea (tensor): dim:(B, C, H, W)
		im2_fea = x[1]   # im2_fea (tensor): dim:(B, C, H, W)
		assert im1_fea.shape[0] == im2_fea.shape[0]
		bs, c, h, w = im1_fea.shape

		# -------------------------------------------------------------------------
		# AvgPooling
		# -------------------------------------------------------------------------
		# AvgPooling for reduce the dimension due to expensive computation
		im1_fea = self.avgpool(im1_fea)
		im2_fea = self.avgpool(im2_fea)

		# -------------------------------------------------------------------------
		# Transformer
		# -------------------------------------------------------------------------
		# pad token embeddings along number of tokens dimension
		im1_fea_flat = im1_fea.view(bs, c, -1)  # flatten the feature
		im2_fea_flat = im2_fea.view(bs, c, -1)  # flatten the feature
		token_embeddings = torch.cat([im1_fea_flat, im2_fea_flat], dim=2)  # concat
		...
		# make contiguous
		im1_fea_out = x[:, 0, :, :, :].contiguous().view(bs, self.n_embd, self.vert_anchors, self.horz_anchors)
		im2_fea_out = x[:, 1, :, :, :].contiguous().view(bs, self.n_embd, self.vert_anchors, self.horz_anchors)

		# -------------------------------------------------------------------------
		# Interpolate (or Upsample)
		# -------------------------------------------------------------------------
		im1_fea_out = F.interpolate(im1_fea_out, size=([h, w]), mode='bilinear')
		im2_fea_out = F.interpolate(im2_fea_out, size=([h, w]), mode='bilinear')

		return im1_fea_out, im2_fea_out
