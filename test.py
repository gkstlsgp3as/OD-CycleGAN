...
from utils.metrics import ap_per_class, ConfusionMatrix, Polygon_ConfusionMatrix
from utils.plots import plot_images, output_to_target, plot_study_txt, set_colors
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            for si, pred in enumerate(out):
                ...
                # Append to text file
                if save_txt:
                    gn = torch.tensor(shapes[si][0])[[1, 0, 1, 0]]  # normalization gain xyxy
                    for *xyxy, conf, cls in predn.tolist():
                        xyxyn = ((torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xyxy
                        line = (cls, *xyxyn, conf) if save_conf else (cls, *xyxyn)  # label format
                    ...
                    if ((wandb_logger.current_epoch > 0) and (wandb_logger.bbox_interval > 1) 
                        ...
                        if batch_i == 0:
                            if wandb_logger.current_epoch == wandb_logger.bbox_interval:
                                ...
                                plot_images(img, targets, colors, paths, f, names)
                            ...
                            plot_images(img, output_to_target(out), colors, paths, f, names)
                            current_batches = [wandb_logger.wandb.Image(str(f), caption=f.name)]
                ...
                if save_json:
                    # [{"image_id": 42, "category_id": 18, "horz_box": [x1, y1, x2, y2], "score": 0.236}, ...
                    image_id = int(path.stem) if path.stem.isnumeric() else path.stem
                    box = predn[:, :4]  # xyxyxyxy
                    for p, b in zip(pred.tolist(), box.tolist()):
                        jdict.append({'image_id': image_id,
                                      'category_id': coco91class[int(p[5])] if is_coco else int(p[5]),
                                      'horz_box': [round(x, 3) for x in b],
                                      'score': round(p[4], 5)})
            ...
            if plots and batch_i < 3:
                ...
                Thread(target=plot_images, args=(img, targets, colors, paths, f, names), daemon=True).start()
                f = save_dir / f'test_batch{batch_i}_pred.jpg'  # predictions
                Thread(target=plot_images, args=(img, output_to_target(out), colors, paths, f, names), daemon=True).start()     
