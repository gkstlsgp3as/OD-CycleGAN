...
def test(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], imgsz, batch_size, gs, opt, pad=0.5, rect=True,
                                       prefix=colorstr(f'{task}: '), polygon=polygon, gan_model=gan_model, task=task)[0]
    seen = 0
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        if fusion == 'early':
            ...
            gan = gan.to(device, non_blocking=True) # [batch, channel, img_shape] e.g. [32, 3, 576, 1056]
            gan = gan.half() if half else gan.float()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fusion', default='mid', help='feature fusion option') # CHange~~
