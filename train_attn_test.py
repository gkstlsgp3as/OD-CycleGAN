...
from utils.torch_utils import adjust_state_dict, modify_optimizer_state_dict
...
def train(hyp, opt, device, tb_writer=None, polygon=False):
    ...
    shutil.copyfile('./utils/cfg.py', save_dir / 'cfg.py')
    shutil.copyfile('./utils/datasets.py', save_dir / 'datasets.py')
    ...
    if rank in [-1, 0]:
        ...
        if opt.resume == False: run_id = None
    ...
    if pretrained:
        ...
        if 'transformer' in opt.cfg.split('/')[-1]:
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            if plots:
                PlotLabels = polygon_plot_labels if polygon else plot_labels
                PlotLabels(labels, colors, names, save_dir, loggers)
                if tb_writer:
                    tb_writer.add_histogram('classes', c, 0)

            # Anchors
            check_anchors_model = check_anchors if not polygon else polygon_check_anchors
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    gans = F.interpolate(gans, size=ns, mode='bilinear', align_corners=False)
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 10:
                    ...
                    f2 = save_dir / f'train_batch{ni}_gan.jpg'  # filename
                    
                    plot_model = polygon_plot_images if polygon else plot_images
                    plot_model(imgs[:, :3, :, :], targets, colors, paths, f)
                    plot_model(imgs[:, 3:, :, :], targets, colors, paths, f2)
...
if __name__ == '__main__':
    ...
    os.environ['WANDB_NOTEBOOK_NAME'] = 'YOLO7_objdt'
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            # Train mutation
            results = train(hyp.copy(), opt, opt.device, polygon=opt.polygon)
